    ...
    private ArrayList<Comment> parseComment(String json) {
        try {
            ...
            for (Map<String, Object> obj : liste) {
                ...
                c.setCreatedAt(obj.get("date").toString());
                 comments.add(c);
            }
     ...
     public boolean addComment(Comment c , int id, String username ) {
        ...
        String Url = Statics.BASE_URL + "/article/"+ id+"/addcom?content=" + c.getContent()+"&user="+username;
        req.setUrl(Url);
     ...
     public ArrayList<Comment> getAllCom() {
        String url = Statics.BASE_URL +"/article/allcomM";
       req.setUrl(url);
        req.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
            public void actionPerformed(NetworkEvent evt) {
